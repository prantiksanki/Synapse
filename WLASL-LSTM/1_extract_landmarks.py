"""
STEP 1 — Extract MediaPipe hand landmarks from WLASL videos.

Usage:
    python 1_extract_landmarks.py \
        --wlasl_dir path/to/WLASL/videos \
        --output_dir MP_Data \
        --num_classes 100 \
        --sequence_length 30
...
def load_wlasl_classes(wlasl_dir: str, num_classes: int) -> dict[str, list[str]]:
    """
    Scans the WLASL video directory.
    Expects structure:  wlasl_dir/<sign_label>/<video>.mp4

    Falls back to reading WLASL2000.json annotation if present.
    Returns {label: [video_path, ...]}
    """
    sign_to_videos: dict[str, list[str]] = {}

    # Try direct folder structure first
    for entry in sorted(os.listdir(wlasl_dir)):
        sign_dir = os.path.join(wlasl_dir, entry)
        if os.path.isdir(sign_dir):
            ...
            if videos:
                sign_to_videos[entry] = videos

    # Trim to requested class count
    selected = dict(list(sign_to_videos.items())[:num_classes])

    if not selected:
        raise FileNotFoundError(
            f"No sign folders with videos found in '{wlasl_dir}'.\n"
            "Expected structure: wlasl_dir/<sign_label>/<video>.mp4"
        )
...
def main():
    ...
    parser.add_argument("--wlasl_dir",       required=True, help="Path to WLASL videos root")
    parser.add_argument("--output_dir",      default="MP_Data", help="Where to save .npy sequences")
    parser.add_argument("--num_classes",     type=int, default=100, help="Number of sign classes to use")
    parser.add_argument("--sequence_length", type=int, default=30,  help="Frames per sequence")
    args = parser.parse_args()

    print(f"\nLoading WLASL from: {args.wlasl_dir}")
    sign_to_videos = load_wlasl_classes(args.wlasl_dir, args.num_classes)
    print(f"Found {len(sign_to_videos)} classes, processing...\n")
